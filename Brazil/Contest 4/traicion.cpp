...
int main(){
    ...
    for (int i = 0; i < 4; i++)
        for(int j = 0; j < 4; j++)
            // razante
            if(i==j){
                double Xg [] ={xg, yg, (x2 - xg),(y2 - yg)}, Xd [] = {xd, xg, (x2 - xd), (y2 - yd)}, Yg [] = {yg, xg, yg, xg}, Yd [] ={yd, xd, yd, xd};
...
cout << (y2 - yg)<<" " << (y2 - yd)<<endl;
    ...
    double tmin = (xg/cos(alp1) + (y2-yd)/cos(alp3))/vw + hypot(xd - (y2 - yd)*tan(alp3), ((y2-yg) - xg*tan(alp1)))/vb;
